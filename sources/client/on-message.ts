import {changePlayer, endGame, initGame, setSendMessage, startGame} from './game.js';

import {AnyServerMessage,} from '../common/messages.js';
...
function processMessage( sendMessage: SendMessage, message: AnyServerMessage ): void
{
	switch ( message.type )
	{
		case 'gameStarted':
			startGame();
			initGame( message.gameState, message.field, message.color );
			break;
		
		case 'changePlayer':
			changePlayer( message.gameState, message.color );
			break;
